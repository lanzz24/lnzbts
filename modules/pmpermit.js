...
async function handler(id) {
    ...
    if (read.status == "not_found") { // Insert 
        ...
        if (insertdb == "insert_error") {
            ...
            var out = ({
                mute: false,
                msg: `*✋ Tunggu*\n\nHarap tunggu sampai saya kembali Online, Mohon jangan kirim pesan lagi.`
            })
            return out
        }
        ...
        if (read.times == 4) {
            var out = ({
                mute: true,
                msg: `*✋ Muted*\n\nYou have been muted, because you are spamming. for ${config.pmpermit_mutetime/60} Minutes for spamming.`
            })
            return out
        } else { // Update times
            ...
            if (update == "update_error") {
                ...
                var out = ({
                    mute: false,
                    msg: `*✋ Tunggu*\n\nHarap tunggu sampai saya kembali Online, Mohon jangan kirim pesan lagi.Kamu Punya Waktu  ${read.times} warning(s).`
                })
                return out
            }
        }
